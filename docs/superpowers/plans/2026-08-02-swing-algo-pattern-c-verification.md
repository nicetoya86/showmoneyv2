# swing-algo-pattern-c-verification Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Determine whether production pattern **C촉매 (촉매이벤트)**, evaluated in isolation
(not pooled with A/B/D), can reach the 90%-hit_rate decision gate on its own — and commit the
honest answer, whatever it is, as reproducible data + an analysis document.
...
`run_one_config` and `select_best_config` are reused exactly as sub-project 7a used them, just
against a candidate list pre-filtered to `pattern_type == "C촉매"` and the same custom 216-cell
grid 7a used.
...
- **Slot-competition caveat** (discovered during sub-project 7a's final review — bake this in from
  the start this time): `run_one_config` calls `apply_daily_selection`
  ...
  C촉매-in-isolation and a pooled result (sub-project 2's A+B+C+D grid, or 7a's A눌림목 result) is
  **not a like-for-like decomposition** — each isolated run's candidates only ever compete against
  themselves for daily slots, so a candidate that would have been crowded out by higher-ranked
  candidates from other patterns in the pooled system can appear in the isolated run instead. Task
  3 must state this explicitly wherever such a comparison appears, and must NOT introduce new
  uncommitted numbers to quantify the crowding-out effect (describe it qualitatively only — every
  number in the document must trace to a committed JSON file).
- **Unmeasured-patterns caveat** (also from 7a's corrected analysis): as of this sub-project,
  **A눌림목's and B지지선's own pooled contributions remain unmeasured** — sub-project 7a
  established A눌림목's isolated profile, but that is not the same as A눌림목's *share* of the
  pooled result (see the slot-competition caveat above for why). B지지선 was superseded by a
  purpose-built replacement pattern (E반등, sub-project 4) that verified E반등, not B지지선 itself.
  Task 3 must not claim this sub-project (plus 7a) constitutes a partial decomposition of the
  pooled system — it adds C촉매's isolated profile to the record, nothing more.
...
### Task 1: Filter and commit the C촉매 candidate pool

**Files:** none created except the output JSON — this is an execution-only task.

- [ ] **Step 1: Filter the existing cache to C촉매 only and save**
...
c_candidates = [c for c in d['candidates'] if c['pattern_type'] == 'C촉매']

out = {'params': d['params'], 'candidates': c_candidates}
json.dump(out, open('backtest_pattern_c_candidates.json', 'w', encoding='utf-8'), ensure_ascii=False)

print('C촉매 count:', len(c_candidates))
print('hold_days distribution:', dict(Counter(c['hold_days'] for c in c_candidates)))
print('date range:', min(c['date'] for c in c_candidates), '..', max(c['date'] for c in c_candidates))
"
```
...
```bash
git add backtest_pattern_c_candidates.json
git commit -m "data(backtest): filter C촉매 candidates for individual pattern verification (sub-project 7b)"
```

---

### Task 2: Grid search C촉매 in isolation, train-select, test-confirm once

**Files:** none created except the output JSON — this is an execution-only task.

**Interfaces:**
- Consumes: `backtest_pattern_c_candidates.json` (Task 1 of this plan), `backtest_regime_lookup.json`.
...
d = json.load(open('backtest_pattern_c_candidates.json', encoding='utf-8'))
...
json.dump(out, open('backtest_pattern_c_grid_results.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
...
```bash
git add backtest_pattern_c_grid_results.json
git commit -m "data(backtest): 216-cell grid search for C촉매 pattern in isolation (sub-project 7b)"
```
...
**Files:**
- Create: `docs/03-analysis/swing-algo-pattern-c-verification.analysis.md`

- [ ] **Step 1: Assemble the honest, fully-cited summary**

Using the real numbers from `backtest_pattern_c_candidates.json` and
`backtest_pattern_c_grid_results.json` (both committed in Tasks 1-2), write
`docs/03-analysis/swing-algo-pattern-c-verification.analysis.md` with these sections (no bracket
placeholders — every number must be the actual value read from these JSON files):

- **Header** matching the convention of
  `docs/03-analysis/swing-algo-pattern-a-verification.analysis.md` (Analysis Type, Project,
  Feature, Design Doc / Implementation Plan links — Design Doc is
  `docs/superpowers/specs/2026-08-02-swing-algo-pattern-c-verification-design.md`, Implementation
  Plan is `docs/superpowers/plans/2026-08-02-swing-algo-pattern-c-verification.md` — Prior work,
  Date `2026-08-02`).
- **Method summary**: restate that this sub-project makes no changes to `evaluate_candidate()` or
  ...
  "C촉매"` (state the actual filtered count from `backtest_pattern_c_candidates.json`) and
  re-runs the existing grid search on a 216-cell grid (state the 4 axes and their values). Note
  briefly that C촉매 is disclosure/supply-event-driven (DART filings, 외국인/기관 순매수),
  qualitatively different from A눌림목's technical pullback trigger — this is context, not a
  finding, so keep it to one sentence.
- **Grid summary table**: from `backtest_pattern_c_grid_results.json`'s `summary` block — grid
...
- **Decision-gate verdict**: state plainly whether C촉매 in isolation reaches
...
- **Comparison to sub-project 7a (A눌림목) and to the pooled sub-project 2 result**: state whether
  C촉매 in isolation looks meaningfully different from A눌림목's isolated result (7a) and from the
  pooled A+B+C+D result — better, worse, or indistinguishable. **This section must open with the
  slot-competition caveat verbatim** (from this plan's Global Constraints): any such comparison is
  not a like-for-like decomposition, because `run_one_config`'s `apply_daily_selection` (max 3/day,
  15/week, ranked by `(grade, rank_score)`) means each isolated run's candidates only compete
  against themselves for slots. Do not introduce new uncommitted numbers to quantify the effect —
  describe it qualitatively. Give an honest trader-perspective read on what the comparison
  (properly caveated) suggests, without overclaiming precision it doesn't have.
- **What remains open**: state plainly that A눌림목's and B지지선's own pooled contributions
  remain separately unmeasured (per this plan's Global Constraints and 7a's corrected analysis),
  that this sub-project adds only C촉매's isolated profile to the record, and that D박스 (7c)
  still remains to be verified — do not claim this sub-project plus 7a constitutes a partial
  decomposition of the pooled system.
...
- **Final recommendation**: state plainly whether this finding changes anything about C촉매's
  current production deployment status (likely "no change recommended, pattern continues running
  as-is pending D박스's result in sub-project 7c" — but base this on the actual numbers found, do
  not assume the answer before reading them).

State explicitly that no production code (`src/swing-scanner.src.js`) was changed by this
sub-project, and that D박스 (sub-project 7c) remains to be verified separately.

- [ ] **Step 2: Commit the analysis document**

```bash
git add docs/03-analysis/swing-algo-pattern-c-verification.analysis.md
git commit -m "docs: final analysis for swing algo sub-project 7b (C촉매 pattern individual verification)"
```
...
- **Spec coverage**: Task 1 covers the design doc's Section 2 (filter, no regeneration, sanity
  ...
  decision gate, slot-competition caveat baked into Global Constraints for downstream use). Task 3
  covers the analysis document requirement implied throughout the design doc (Section 1's goal
  statement, Section 3's caveat, Section 7's limitations), and explicitly carries forward both
  lessons from 7a's final review (slot-competition caveat stated up front, unmeasured-patterns
  caveat) rather than leaving them to be rediscovered.
...
- **Type consistency**: `CachedCandidate` field names and `run_one_config`/`select_best_config`'s
  ...
  throughout every task — identical to sub-project 7a's plan, no invented names.
