...
| **Solution** | 전일대비 임계값 상수 추가 + 2단계 필터 (절대 상한 10% 초과 시 차단, 8% 이상 시 최소 점수 180점 요구). |
...
| FR-02 | 전일대비 8~10% 구간 + 점수 < 180 → 진입 차단 (270→180 현실화, 2026-05-28) | High | LS네트웍스·SIMPAC 손절 방지 |
...
| SIMPAC +9.3% → 손절 | 차단 (점수 238 < 180 기준: **통과** — 실증 데이터로 기준 완화) |
| LS네트웍스 +9.8% → 손절 | 차단 (점수 250 > 180 → **통과됨**, 판단 유보) |
| 상도어메니티 +9.8% → 수익 | 통과 (점수 255 > 180) ← 수익 종목 포착 |
| *180점 미만 고급등 종목* | 차단 — 고점수 없이 8%+ 진입 차단 유지 |
...
const SURGE_ZONE_MIN_SCORE  = 180;   // 급등 구간 최소 통과 점수 (270→180 현실화, 2026-05-28)
// ===== /전일 급등 진입 억제 상수 =====
```
...
if (prevChange > SURGE_ZONE_PCT && score < SURGE_ZONE_MIN_SCORE) return; // +8%+ 180점 미만 차단
```
