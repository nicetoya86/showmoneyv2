const run = async function () {
  ...
  // [TEST MODE]
  if (forceTest) {
    const msg = '🌊 [스윙 스캔 테스트] 시스템 정상 작동 ' + NL + '⏰ ' + kst.toISOString().slice(0,16).replace('T',' ') + ' KST';
    try { 
  ...
  if (h > 15 || (h === 15 && m > 0)) return [{ json: { skipped: true, reason: 'After market close' } }];
  ...
  const cleanOldHistory = () => {
    ...
    for (const ticker in store.swingSent) { if (store.swingSent[ticker] < cutoff) delete store.swingSent[ticker]; }
  };
  cleanOldHistory();
  ...
  const send = async (c)=>{
    ...
    const msg = icon + ' [장중 스윙 포착] ' + c.market + ' | ' + esc(c.name) + ' (' + c.code + ')' + NL
    ...
    try { 
      await http({ method:'POST', url:'https://api.telegram.org/bot'+BOT+'/sendMessage', json:true, body:{ chat_id:CHAT, text:msg, parse_mode:'HTML' } }); 
  ...
  for (let i=0; i<selected.length; i++){
    ...
    if (res){
      ...
      if (!store.weeklyRecommendations) store.weeklyRecommendations = {};
      if (!store.weeklyRecommendations[today]) store.weeklyRecommendations[today] = [];
      store.weeklyRecommendations[today].push({ type:'swing', subType:selected[i].type, ticker:selected[i].ticker, code:selected[i].code, name:selected[i].name, entry:res.entry, target:res.target, stop:res.stop, holdingDays:1, score:selected[i].score });
