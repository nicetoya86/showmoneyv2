...
// 수정일: 2026-07-10
// 변경사항: KRX 정리매매/투자경고/투자위험 3종을 토스 Open API /warnings로 대체
//           (관리종목/투자주의/투자주의환기/거래정지는 기존 KRX best-effort 유지)
// 참고: docs/02-design/features/risk-blacklist-toss-api.design.md
// 배포 상태: 미배포 — 로컬 구현본 (라이브 n8n Risk Blacklist Updater 노드는 아직 미반영)
// ========================================
...
async function notify(text) {
  try {
    await http({ method: 'POST', url: 'https://api.telegram.org/bot' + BOT + '/sendMessage', json: true, body: { chat_id: CHAT, text } });
  } catch (e) {}
}
...
// [TOSS-RISK] 정리매매/투자경고/투자위험 3종은 토스 /warnings 로 대체 — 아래 목록에서 제외 (2026-07-10)
...
const RISK_WARNING_TYPES = new Set(['LIQUIDATION_TRADING', 'INVESTMENT_WARNING', 'INVESTMENT_RISK']);
const TOSS_WARNINGS_CONCURRENCY = 6;

function _tossApiKey() {
  return store.tossApiKey || '';
}

async function fetchTossAPI(endpoint) {
  const apiKey = _tossApiKey();
...
async function fetchTossWarnings(symbols) {
  const codes = new Set();
  const result = { codes: [], errorCount: 0, checked: 0, skippedNoKey: false };
  ...
  const { errors } = await mapLimit(symbols, TOSS_WARNINGS_CONCURRENCY, async (symbol) => {
    result.checked++;
    const resp = await fetchTossAPI('/api/v1/stocks/' + symbol + '/warnings');
...
try {
  ...
  await notify('✅ [리스크 블랙리스트 갱신 성공]' + NL +
    ...
    'Toss(정리매매·경고·위험): ' + tossStatus + ' ' + tossResult.codes.length + '개 (검사 ' + tossResult.checked + '개)' + NL +
